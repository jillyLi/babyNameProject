...
<body>
	<h1>Change ${thisBabyName.newName}</h1>
	<form:form action="/babyName/edit/${thisBabyName.id}" method="post" modelAttribute="thisBabyName">
		<input type="hidden" name="_method" value="put"/>
		<p>
	        <form:label path="newName">New Name:</form:label>
	        <form:errors  path="newName" class="error"/>
	        <form:input path="newName" readOnly="true"/>
	    </p>
	    <p>
		...
		<form:input type="hidden" path="votes" value="${thisBabyName.votes}"/>
		<form:input type="hidden" path="owner" value="${thisBabyName.owner}"/>
		<%--  <form:input type="hidden" path="users" value="${thisUser.id }"/> --%>
	
		<a href="/babyName/delete/${thisBabyName.id }">Delete</a>&emsp;&emsp;&emsp;&emsp;
